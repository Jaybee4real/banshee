...
public static class Program
{
    ...
    {
        using var instanceLock = new Mutex(true, "BansheeSingleInstance", out var isFirstInstance);
...
public class TrayApp : ApplicationContext
{
    ...
    public TrayApp()
    {
        var config = BansheeConfig.Load();
        watcher = new Watcher(config);
        watcher.AlarmRequested += ShowAlarm;
        watcher.StateChanged += RefreshMenu;

        statusItem = new ToolStripMenuItem("BANSHEE — starting…") { Enabled = false };
        ...
        menu.Items.Add(new ToolStripSeparator());
        menu.Items.Add(new ToolStripMenuItem("Quit BANSHEE", null, (_, _) => Quit()));
    ...
    private void RefreshMenu()
    {
        ...
        {
            statusItem.Text = "BANSHEE — ALARM ACTIVE";
            trayIcon.Icon = SystemIcons.Error;
        }
        else if (watcher.Armed)
        {
            statusItem.Text = "BANSHEE — Armed";
            trayIcon.Icon = SystemIcons.Shield;
        }
        else
        {
            statusItem.Text = config.AutoArmDaily
                ? $"BANSHEE — Disarmed · arms at {config.ArmHour:D2}:{config.ArmMinute:D2}"
                : "BANSHEE — Disarmed";
    ...
    private void ShowAlarm(string reason)
    {
        ...
        alarmForm = new AlarmForm(BansheeConfig.Load(), reason);
        alarmForm.Disarmed += () =>
        {
            alarmForm = null;
    ...
    private void DisarmWithPin()
    {
        ...
        if (config.VerifyPin(dialog.CurrentPin))
            ...
            MessageBox.Show("Wrong code.", "BANSHEE", MessageBoxButtons.OK, MessageBoxIcon.Error);
    }
    ...
    private void Drill()
    {
        var confirm = MessageBox.Show(
            ...
            "BANSHEE Drill", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
    ...
    private void ShowFirstRun()
    {
        ...
        dialog.Text = "Welcome to BANSHEE — set your disarm code";
        while (true)
        {
            if (dialog.ShowDialog() != DialogResult.OK) continue;
            if (dialog.NewPin.Length >= 4 && dialog.NewPin == dialog.ConfirmPin) break;
            MessageBox.Show("Codes must match and be at least 4 characters.", "BANSHEE",
    ...
    private void Quit()
    {
        if (watcher.Armed)
        {
            var config = BansheeConfig.Load();
            using var dialog = new PinDialog(true);
            dialog.Text = "BANSHEE is armed — enter code to quit";
